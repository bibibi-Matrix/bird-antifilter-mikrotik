...
- **Чёрный список** — IP/CIDR из папки `black_list/` исключаются из финальных `.rsc` файлов; если запись чёрного списка попадает внутрь более широкого префикса, префикс **пересчитывается и разбивается** на существующие части (например, удаление одного IP из `/24` разбивает диапазон на 2) — для IPv4 и IPv6
...
├── bin/
│   ├── bird2.sh                   # Основной скрипт синхронизации (v3.3.x)
...
blacklist_split=yes        # Разбивать широкие префиксы при попадании в них записей чёрного списка
...
Применяется на этапе формирования `.rsc` (шаг 5). Записи, попадающие внутрь более широкого префикса, **вычитаются**: префикс пересчитывается и разбивается на части (для IPv4 и IPv6). Отключить разбиение можно флагом `blacklist_split=no` — тогда удаляются только маршруты, целиком покрываемые чёрным списком. `0.0.0.0/0` отклоняется (удалил бы все маршруты).
...
5. **apply_blacklist** — удаляет маршруты, попадающие под `black_list/`, и разбивает более широкие префиксы, если запись чёрного списка лежит внутри них
...
Каждый запуск завершается **статистическим отчётом** (кол-во скачанных списков, невалидных CIDR, исключённых blacklist-маршрутов и разбиений префиксов, удалённых перекрытий и общее число маршрутов).
...
| Версия | Описание |
|--------|----------|
| v3.3.0 | Blacklist: пересчёт и разбиение префиксов при попадании записей чёрного списка внутрь более широких диапазонов (IPv4/IPv6, hex-trie без 128-бит арифметики), глобальная дедупликация `.rsc` после разбиения, флаг `blacklist_split` |
